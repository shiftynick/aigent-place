...
pass/fail signal — one command that goes **red** on *this* bug — you will find
...
1. **Failing test** at whatever seam reaches the bug (target the test
   suite that actually contains the code path — check where tests live
   for the component or package in question).
2. **HTTP script** against a running dev server (`curl` or the project's
   standard HTTP client, with whatever auth the dev setup uses).
3. **CLI invocation** with a fixture input, diffing output against a
   known-good snapshot.
4. **Browser script** — for UI bugs, drive a real browser and assert on
   DOM/console/network. Rebuild first if the server embeds assets, or
   you'll browser-verify stale code.
...
10. **HITL script.** Last resort. If a human must click, drive *them* with
    `node .agents/skills/diagnosing-bugs/scripts/hitl-loop.template.mjs` so the
    loop is still structured and the captured output feeds back to you.

If the project has an end-to-end smoke script, it's a ready-made loop for
bugs in boot/seed/smoke territory — but narrow it: a smoke pass that goes
red on *anything* is not a signal for *this* bug.
...
to whatever environment reproduces it, (b) a captured artifact (HAR, log
dump, DB copy, screen recording with timestamps), or (c) permission to add
...
inputs, callers, config, data, and steps one at a time, re-running the loop
...
hypothesis space and becomes the Phase 5 regression test.
...
worse." If you cannot state the prediction, the hypothesis is a vibe —
...
Write the regression test **before the fix** — but only if a **correct
seam** exists: one where the test exercises the real bug pattern as it occurs
at the call site. If the only available seam is too shallow (a unit test that
can't replicate the chain that triggered the bug), a regression test there
gives false confidence.

**If no correct seam exists, that itself is the finding.** Document it and
file a follow-up task with the `task-tracker` skill for the structural fix.

If a correct seam exists: turn the minimised repro into a failing test at
that seam → watch it fail → apply the fix → watch it pass → re-run the
...
**Then ask: what would have prevented this bug?** If the finding
generalizes to a defect *class*, DISTILL it into
`docs/REVIEW-STANDARDS.md` (per that file's format). If the answer is
architectural (no good seam, tangled callers, hidden coupling), file a
board task — after the fix is in, not before; you know more now.
